 ...
 * @Description json报文将参数转换为大写下划线
 */
public class ObjToJsonUpperCase {
    ...
    public static JSONObject getJsonMsg(User parent , List<User> childTree){
        ...
        if("String".equals(parentType)){
            innerMap.put(parent.getName().toUpperCase(),parent.getValue());
        }
        if("JSONObject".equals(parentType)){
            ...
            innerMap.put(parent.getName().toUpperCase(),jo);
        }
        if("JSONArray".equals(parentType)){
            ...
            innerMap.put(parent.getName().toUpperCase(),array);
        }
